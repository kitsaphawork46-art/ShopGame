...
const links = [
  ["หน้าแรก", "#top"], ["ร้านค้า", "#products"], ["เกมทั้งหมด", "#games"],
  ["วิธีการสั่งซื้อ", "#how-it-works"], ["รีวิว", "#reviews"], ["ติดต่อเรา", "#footer"],
];
    ...
    <header className={`fixed inset-x-0 top-0 z-50 border-b transition-all ${scrolled ? "border-slate-800/90 bg-ink/90 shadow-xl backdrop-blur-xl" : "border-transparent bg-ink/45 backdrop-blur-sm"}`}>
      <nav className="container-page flex h-[74px] items-center justify-between gap-6" aria-label="เมนูหลัก">
        ...
        <div className="flex items-center gap-1.5 sm:gap-2">
          ...
          <Link href="/register" className="focus-ring hidden rounded-xl bg-brand px-4 py-2.5 text-sm font-bold text-white shadow-[0_8px_24px_rgba(37,99,235,.25)] transition hover:bg-blue-500 md:block">สมัครสมาชิก</Link>
